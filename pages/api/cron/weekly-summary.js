...
// Fetches the past 7 days of reviews across ALL locations and sends a summary.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const overallBreakdown = [5, 4, 3, 2, 1].map(stars => {
    ...
    const locationSections = LOCATIONS.map(loc => {
      ...
      if (locReviews.length === 0) return `${loc.label}: no reviews`;
      const locAvg = (locReviews.reduce((s, r) => s + r.starRating, 0) / locReviews.length).toFixed(1);
      const locReplied = locReviews.filter(r => r.reviewReply).length;
      return `${loc.label}: ${locReviews.length} reviews, ${locAvg}⭐, ${locReplied}/${locReviews.length} replied`;
    }).join('\n  ');

    const summary = `
Contrabando — Weekly Review Summary (All Locations)
Week ending: ${new Date().toLocaleDateString('en-GB')}

Overall: ${reviews.length} reviews, ${avgRating}⭐ avg, ${replied}/${reviews.length} replied (${responseRate}%)
...
`.trim();
    ...
    if (alertEmail && process.env.SMTP_HOST) {
      ...
      await transporter.sendMail({
        ...
        to: alertEmail,
        subject: `Weekly Review Summary — Contrabando (${reviews.length} reviews, ${avgRating}⭐)`,
        text: summary
      });
